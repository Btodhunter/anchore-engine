...
print("accounts-url: " + base_url)
print("accounts-auth: " + base_auth[0] + ", " + base_auth[1])
...
class SimpleClient(object):
    ...
    def create_account(self, account_name, account_type):
        ...
        body = {
            'name': account_name,
            'email': account_name + "@testaccount.com"
        }
...
def assert_account_state(resp, state_str):
    ...
    found = j.get('state')
    if found != state_str:
        raise AssertionError('Expected account state: {}, found {}'.format(state_str, found))
    else:
